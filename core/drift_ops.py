...
Живе в `core/`, а не в `scripts/`, з тієї ж причини, що й `core/sync_ops.py`:
цю логіку викликає ще й `api/admin_routes.py` (owner-панель), а `scripts/`
навмисно не потрапляє в прод-контейнер (.gcloudignore).

Нічого не змінює. Порівнює три стани:
...
from core.sync_ops import SYNC_STATE, actual_from_st, owned_values
...
async def main(argv: list[str] | None = None):
    ...
    адмін-панель викликає з явним списком (напр. `[]`)."""
